...
from includes.patterns import Patterns  #pylint: disable=wrong-import-position
from includes.plotting import PatPlot  #pylint: disable=wrong-import-position

# Initialize an object from the calss
DATA = Patterns()

# Example usage of the analytical pattern generation with defaults - half \
# lambda dipole
DATA.generate_analytical_pattern()  # this will be pattern index 0

# Example usage of the analytical pattern generation with defaults - one \
# lambda dipole
...
DATA.load_data("CST_File", os.path.join('example', 'dipole', 'Export',
                                        'Farfield', 'farfield (f=1000) [1].txt'
                                        ))
# this will be pattern index 2

# Example loading all files in a CST export folder from the dipole example
DATA.load_data("CST_Folder", os.path.join('example','dipole','Export','Farfield'))
...
#DATA.load_data("CST_Par_Sweep", os.path.join('example','dipole'))

# The sweep defined in the example dipole file is
# of lambda 1/2, 1 and 2 for the dipole length, each producing three Patterns on export. This makes
# a total of 9 additional Patterns appended after index 5.

# Example loading a Satimo measuremnt file
DATA.load_data("AAU_Satimo", os.path.join('example','CSL_HB_detailed.txt'))

# Example loiading from JSON file
#DATA.load_data('json', "example\\example_json.json")

# Example storing to JSON
includes.data_load_store.save_to_json(DATA, os.path.join('example','example_json.json'))
...
# Setting the statistics to ommit the poles within 10 degrees - makes some sense for dipoles
FIELD_TO_ANALYZE = 'Gabs'
...
## Export to Exel
includes.data_load_store.save_to_excel(DATA, [0,1])
