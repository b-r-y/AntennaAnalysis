...
class Patterns():
    '''
    Main Patterns class. An object of this class contains a list of ``Patterns`` with each member
    of the list being an ordered dictionary object.

    Attributes:
        comment_static_vars (str): Comment string used as a header in the JSON file export.
            Variables in this section are static.
        version (float): version of the ``Patterns`` structure
        comment_auto_populated (str): Comment string used as a header in the JSON file export.
            Variables in this section are autogenerated from file imports.
    ...
    def load_data(self, input_type, input_file_or_folder):
        ...
        elif input_type == 'AMS32':
            self = includes.data_load_store.load_from_ams32(self, input_file_or_folder)
    ...
    def fetch_field_with_context(self, pat_ind, field = 'Gabs', field_format = 'dB', \
                                 rot_angles = None):
        '''
        Same behaviour as :func:`~includes.patterns.fetch_field` but with an additional context \
        output.
        '''
    ...
    def fetch_field(self, pat_ind, field = 'Gabs', field_format = 'dB', rot_angles = None):
        ...
        Standard spherical coordinate system and angular rotations of antenna are defined in
        [IEEESTDTerms]_ and [IEEESTDTest]_.
        ...
        Arguments:
            ...
            field (str): the type of field requested. If the requested field is different from the
                ...
                :func:`~includes.field_operations.convert_field` for list of possible values.

            field_format (str): The format of the output field. Definitions are according to the \
                Touchstone file format. See :func:`~includes.field_operations.convert_field_format`\
                 for details on options.

            rot_angles (int,int,int): list of three Euler rotation angles describing the rotation.
                The definitions are according to [IEEESTDTest]_. The order is --> [yaw, pitch, roll]
    ...
    def get_field_stats(self, pat_ind, field='Gabs', analysis_range = None, rot_angles=None):
        ...
        Arguments:
            ...
            field (str): field selected for analysis. The field is selected via the strings defined\
                in :meth:`~includes.field_operations.convert_field`. The default field is ``Gabs``.
            ...
            rot_angles (int,int,int): defines the rotation Euler angles for the pattern to be \
                ...
                coordiante system as defined by IEEE - see [IEEESTDTest]_ and [IEEESTDTerms]_. \
        ...
        Returns:
            tuple: four values with the analysis data in linear scale
                ...
                - tot_field_power (float): is the total field power as defined in \
                    :meth:`~incldues.patterns.get_tot_field_power`. This is always calculated for \
                    the full 3D sphere.
    ...
    def compute_antenna_temperature(self, pat_inds, brighness_model = None):
        '''
        ...
        for ind in pat_inds:
            ...
            brightness_temperature = \
                includes.field_operations.generate_brightness_temperature(brighness_model,\
                                                                          g_abs.shape)
            ant_temp.append(includes.field_operations.antenna_temperature_calculation(\
