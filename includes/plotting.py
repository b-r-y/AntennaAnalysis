...
class PatPlot():
    ...
    def __init__(self, Patterns, legends):
        ...
        self.plot_settings = {'plot_type': 'polar',
                              'scale': 'log',
                              'dynamic_range': None,
    ...
    def __apply_plot_range_limit(plot_range, pattern):
        ...
        Arguments:
            ...
            pattern (array): array witht eh fields data
        ...
        th_step, ph_step = includes.misc.get_angle_steps(pattern, sphere_type = 'closed')

        selection_matrix = np.full(pattern.shape, np.finfo(float).eps)
        selection_matrix[int(plot_range[0][0]//th_step):int((plot_range[0][1]+th_step)//th_step) , \
                         int(plot_range[1][0]//ph_step):int((plot_range[1][1])+ph_step//ph_step)] \
                         = 1
    ...
    def __process_data_select_inputs(data_select):
        ...
        if data_select.__class__ == str:
            if data_select == 'XZ':
                ...
                sys.exit('Undefined high level data_select option! --> {}'.format(data_select))
        else:
            ...
            info_str = 'Sweeping ' + plot_angle + ' @ ' + other_angle + ' values' + \
                plot_vector.__str__()
    ...
    def __get_plot_data(pattern, plot_angle, curr_angle):
        '''
        This method fetches the requested slices from teh pattern to extract only the data needed
        for a specific plot.
    ...
    def __set_xaxis(axes, plot_angle, spacing = 30, linear_plot_offset = 0):
        ...
        if plot_angle == 'theta':
            xlabels = [str(int(x)) for x in np.linspace(0, 180, no_samples).tolist() + \
                                            np.flip(np.linspace(0, -180+spacing, no_samples-1)\
                                            ,axis=0).tolist()]
            xlabels[180//spacing] = r'$\pm 180$'
        elif plot_angle == 'phi':
            xlabels = [str(int(x)) for x in np.linspace(0, 360, 2*no_samples - 1).tolist()]
    ...
    def __apply_linear_plot_offset(plot_axis, selected_data, linear_plot_offset):
        ...
        plot. Should not be used independdently.
        '''
    ...
    def __apply_dynamic_range_limit(self, plot_curves):
        ...
        range], scale]
            ...
            - the second place in teh tuple can be either a single value defining the range in the\
                ...
                strategy.
            -  the third value int eh tuple can be eitehr 'log' or 'lin' specifying logarithmic or\
                linear scale for the dynamic range definitions. Note that this is independent of \
                the actual scale for teh plot.
    ...
    def __apply_scale(self, field_index, field_data):
        ...
        '''
        if self.plot_settings.get('scale') == 'log':
            field_data = includes.misc.lin2log(self.legends[field_index][0], field_data)
        return field_data
    ...
    def __get_standard_plot_axis(self, plot_angle):
        ...
        if plot_angle == 'theta':
            plot_axis = np.concatenate((np.linspace(0,180,no_samples[0]),\
    ...
    def __augment_legends(self,plot_angle, plot_vector):
        ...
        for i_0 in range(self.legends.__len__()):
            for i_1 in range(plot_vector.__len__()):
                plot_legends.append('[' + other_anlge + '=' + str(plot_vector[i_1]) + 'deg] --> ' \
    ...
    def __plot_standard_2d(self, linear_plot_offset, plot_angle, plot_vector, ax1):
        '''
        Plots the standard polar or linear plaots.
        '''
        data_to_plot = []
    ...
    def __plot_2d_contour(self, ax1, ind, n_bins, linear_plot_offset):
        ...
        vth, vph = includes.misc.get_angle_vectors(data_to_plot, 'closed')
    ...
    def __save_figure(self, figure_file_name, fig, extra_artists=None):
        ...
        fig.savefig(save_folder + '/' + figure_file_name + '.png', \
                            bbox_extra_artists=(extra_artists, ), \
                            bbox_inches='tight')
        fig.savefig(save_folder + '/' + figure_file_name + '.pdf', \
                            bbox_extra_artists=(extra_artists, ), \
                            bbox_inches='tight')
        fig.savefig(save_folder + '/' + figure_file_name + '.svg', \
                            bbox_extra_artists=(extra_artists, ), \
                            bbox_inches='tight')

    def set_plot_params(self, plot_settings):
        '''
        ...
        '''
        self.plot_settings = includes.misc.update_params(self.plot_settings, plot_settings)
        ...
        if self.plot_settings.get('plot_type') == 'linear':
            if plot_angle == 'theta':
                lim = self.__get_wraparound_axis_limits(
                    np.array(self.plot_settings.get('plot_range')[0]) + linear_plot_offset,
                    360 - np.array(self.plot_settings.get('plot_range')[0]) + linear_plot_offset)
            elif plot_angle == 'phi':
                lim = np.array(self.plot_settings.get('plot_range')[1]) + linear_plot_offset
                if all(lim >= 360):
                    lim = lim - 360
                elif any(lim > 360):
    ...
    def make_standard_plot(self, data_select = 'XZ', xtick_spacing = 30, linear_plot_offset = 0):
        '''
        ...
        Arguments:
            data_select (tuple): describes the slice of the full sphere to be plotted. The first\
                ...
                Short hand notations for the IEEE primary planes can aslo be defined as strings -\
        ...
        '''
        info_str, plot_angle, plot_vector = self.__process_data_select_inputs(data_select)
        fig = plt.figure()
        ax1 = self.__make_axes(fig, plot_angle)
        info_str,_ = self.__make_title(ax1, info_str)

        y_plot_limits = self.__plot_standard_2d(linear_plot_offset, plot_angle, plot_vector, ax1)
        ...
        lgd = ax1.legend(lgd,loc='upper center', bbox_to_anchor=(0.5, -0.05), \
        ...
        self.__save_figure(info_str + ' ' + self.plot_settings.get('plot_type'), fig, lgd)

        plt.show()

        return fig

    def make_statistics_plot(self, stat_plot_type = 'CDF'):
        '''
        Makes a CDF or CCDF plots fo the data in the PatPlot object.

        Arguments:
            stat_plot_type (str): type of the plot. Possible values are 'CDF' (default) and 'CCDF'
        ...
        data_to_plot = []
        for idx, curr_p in enumerate(self.patterns):
            curr_p = includes.misc.open_sphere(curr_p)
            curr_p = includes.misc.apply_analysis_range(curr_p, \
                                                        self.plot_settings.get('plot_range'))
            sorted_data = np.sort(curr_p.ravel())
    ...
    def make_countour_plot(self, ind, n_bins = 30, tick_spacing = None, linear_plot_offset = None):
        '''
        ...
        lgd = ax1.legend([plt.Rectangle((0,0),1,1)],[' '.join(self.legends[ind])],\
                         loc='upper center', \
                         bbox_to_anchor=(0.5, -0.10), fancybox=True, shadow=True, ncol=1)
        box = ax1.get_position()
        ax1.set_position([box.x0, box.y0 + box.height * 0.1, box.width, box.height * 0.8])
